...
- The current application has a playable core tournament loop, but it is not yet
  a full manager career or match-center experience.
- Teams, groups, fixtures, tournament rules, official squads, players, a
  database migration, internally generated seeds, estimated ratings, default
  lineups, and headless match simulation are present.
- Core game flow is playable through `/play` with selected-team match pacing and
  background simulation up to the next selected-team fixture.
...
- Only the 11 June 2026 FIFA ranking is populated. The standings engine supports
  ...
  resolve it.
- No authentication or server-side user save storage exists; browser IndexedDB
  saves are implemented.
...
- User tactics editing, legal lineup editing, full calendar management, and
  simultaneous-match presentation are still simplified.
